...
pub(crate) fn padding_as(method: &str, block_size: usize, data: &[u8]) -> Vec<u8> {
  match method {
    ...
    "space" => if data.len() % block_size == 0 {
      data.to_vec()
    } else {
      let mut out = data.to_vec();
...
pub(crate) fn aes_ecb_action(is_enc: bool, input_data: &[u8], key: Option<&[u8]>, padding: &str ) -> Result<Vec<u8>> {
...
pub(crate) fn aes_cbc_action(is_enc: bool, input_data: &[u8], key: Option<&[u8]>, padding: &str ) -> Result<Vec<u8>> {
  ...
  if is_enc {
    ...
    let msg_len = out.len();
    cipher.encrypt_padded_mut::<aes::cipher::block_padding::NoPadding>(out.as_mut_slice(), msg_len)
    .map_err(|e| anyhow!("{}", e))?;
    ...
    cipher.decrypt_padded_mut::<aes::cipher::block_padding::NoPadding>(&mut out)
    .map_err(|e| anyhow!("{}", e))?;
    Ok(out)
  }
